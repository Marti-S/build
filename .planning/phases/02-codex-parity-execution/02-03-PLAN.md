...
<objective>
Verify Codex vs OpenCode parity by running two representative /gsd commands in isolated workspaces and comparing artifacts.
...
- Create `.planning/parity/02-parity-report.md` with sections for runs, artifact paths, and diff results for both `/gsd:plan-phase` and `/gsd:execute-phase`.
  </action>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    1. In `.planning/parity/codex-run`, run `/gsd:plan-phase <parity-phase> --skip-research --skip-verify`.
    2. In `.planning/parity/opencode-run`, run `/gsd-plan-phase <parity-phase> --skip-research --skip-verify`.
    3. In `.planning/parity/codex-run`, run `/gsd:execute-phase <parity-phase>`.
    4. In `.planning/parity/opencode-run`, run `/gsd-execute-phase <parity-phase>`.
    5. Confirm each workspace creates `.planning/phases/<parity-phase>-*/` with a PLAN.md and SUMMARY.md.
  </how-to-verify>
...
<task type="auto">
  ...
  <action>
Compare the generated artifacts and record the results for both commands.
...
- Note parity separately for the plan-phase artifacts (PLAN.md) and execute-phase artifacts (SUMMARY.md and any execution metadata).
...
- Both `/gsd:plan-phase` and `/gsd:execute-phase` outputs are included in the parity report.
- Parity report lists both artifact paths and diff results.
</verification>

<success_criteria>
- `.planning/parity/02-parity-report.md` documents parity results for both `/gsd:plan-phase` and `/gsd:execute-phase`.
